...
%   defaultvalue    Standardwert fuer nicht durch Boxen definierte Bereiche
%
%   Rueckgabe
%   vec             Vektor der diskretisierten Verteilung
...
function [vec] = boxMesher(msh, boxes, defaultvalue)
    ...
    for box=boxes
        ...
        inds = xmin:xmax;
        inds = repmat(inds, [diffy 1])' + repmat(((ymin:ymax)-1)* My,...
                     [diffx 1]);
        inds = reshape(inds, [1 diffx*diffy]);
        inds = repmat(inds, [diffz 1])' + repmat(((zmin:zmax)-1)* Mz,...
                     [diffx*diffy 1]);
